...
class EmccOptions:
  def __init__(self):
    ...
    # Any subdirectory of the emscripten root as a valid abspth for -I and -L
    self.valid_abspaths = [utils.path_from_root()]
    self.cfi = False
...
def emsdk_cflags(user_args):
  ...
  return cflags + ['-Xclang', '-iwithsysroot/include/compat']
...
def parse_args(newargs):
  ...
  for i in range(len(newargs)):
    ...
    if arg.startswith('-O'):
      ...
      options.valid_abspaths.append(pathlib.Path(consume_arg()))
      ...
      path_name = pathlib.Path(arg[2:])
      if path_name.is_absolute() and not any(v in path_name.parents for v in options.valid_abspaths):
