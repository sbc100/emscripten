...
IMPORT_IMPL(void, Z_wasi_snapshot_preview1Z_proc_exit, (u32 x), {
  exit(x);
});
...
IMPORT_IMPL(u32, Z_envZ___sys_open, (u32 path, u32 flags, u32 varargs), {
...
IMPORT_IMPL(u32, Z_wasi_snapshot_preview1Z_fd_write, (u32 fd, u32 iov, u32 iovcnt, u32 pnum), {
...
IMPORT_IMPL(u32, Z_wasi_snapshot_preview1Z_fd_read, (u32 fd, u32 iov, u32 iovcnt, u32 pnum), {
  int nfd = get_native_fd(fd);
...
IMPORT_IMPL(u32, Z_wasi_snapshot_preview1Z_fd_close, (u32 fd), {
...
IMPORT_IMPL(u32, Z_wasi_snapshot_preview1Z_environ_sizes_get, (u32 pcount, u32 pbuf_size), {
...
IMPORT_IMPL(u32, Z_wasi_snapshot_preview1Z_environ_get, (u32 __environ, u32 environ_buf), {
  // TODO: connect to actual env?
  return 0;
});
...
IMPORT_IMPL(u32, Z_wasi_snapshot_preview1Z_fd_seek, (u32 fd, u64 offset, u32 whence, u32 new_offset), {
  int nfd = get_native_fd(fd);
...
IMPORT_IMPL(u32, Z_envZ___sys_unlink, (u32 path), {
  VERBOSE_LOG("  unlink %s\n", MEMACCESS(path));
  if (unlink(MEMACCESS(path))) {
    VERBOSE_LOG("    error, %d %s\n"u errno, strerror(errno));
    return EM_EACCES;
  }
  return 0;
});
...
IMPORT_IMPL(u32, Z_envZ___sys_fstat64, (u32 fd, u32 buf), {
  int nfd = get_native_fd(fd);
...
IMPORT_IMPL(u32, Z_envZ___sys_stat64, (u32 path, u32 buf), {
...
IMPORT_IMPL(u32, Z_envZ___sys_read, (u32 fd, u32 buf, u32 count), {
  int nfd = get_native_fd(fd);
...
IMPORT_IMPL(u32, Z_envZ___sys_access, (u32 pathname, u32 mode), {
...
IMPORT_IMPL(u32, Z_wasi_snapshot_preview1Z_clock_time_get, (u32 clock_id, u64 max_lag, u32 out), {
...
IMPORT_IMPL(u32, Z_wasi_snapshot_preview1Z_clock_res_get, (u32 clock_id, u32 out), {
  if (!check_clock(clock_id)) {
    return WASI_EINVAL;
  }
