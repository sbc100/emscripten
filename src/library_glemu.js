...
var LibraryGLEmulation = {
  ...
  $GLEmulation: {
    ...
    init() {
      ...
      var glGetBooleanv_real = glGetBooleanv;
      ...
      var glGetString_real = glGetString;
...
#endif
      var glCreateShader_real = glCreateShader;
      glCreateShader = emscripten_glCreateShader = (shaderType) => {
        var id = glCreateShader_real(shaderType);
        GL.shaderInfos[id] = {
          type: shaderType,
          ftransform: false
        };
      ...
      var glShaderSource_real = glShaderSource;
      ...
      var glCompileShader_real = glCompileShader;
      ...
      GL.programShaders = {};
      var glAttachShader_real = glAttachShader;
      glAttachShader = emscripten_glAttachShader = (program, shader) => {
        GL.programShaders[program] ||= [];
        GL.programShaders[program].push(shader);
        glAttachShader_real(program, shader);
      };

      var glDetachShader_real = glDetachShader;
      glDetachShader = emscripten_glDetachShader = (program, shader) => {
        ...
        glDetachShader_real(program, shader);
      };

      var glUseProgram_real = glUseProgram;
      ...
      var glDeleteProgram_real = glDeleteProgram;
      ...
      var glBindAttribLocation_real = glBindAttribLocation;
      glBindAttribLocation = emscripten_glBindAttribLocation = (program, index, name) => {
        if (index == 0) zeroUsedPrograms[program] = true;
        glBindAttribLocation_real(program, index, name);
      };

      var glLinkProgram_real = glLinkProgram;
      ...
      var glBindBuffer_real = glBindBuffer;
      ...
      var glGetFloatv_real = glGetFloatv;
      glGetFloatv = emscripten_glGetFloatv = (pname, params) => {
        ...
        if (pname == 0xBA6) { // GL_MODELVIEW_MATRIX
          ...
          glGetFloatv_real(pname, params);
        }
      };

      var glHint_real = glHint;
      glHint = emscripten_glHint = (target, mode) => {
        if (target == 0x84EF) { // GL_TEXTURE_COMPRESSION_HINT
          return;
        }
        glHint_real(target, mode);
      };

      var glEnableVertexAttribArray_real = glEnableVertexAttribArray;
      glEnableVertexAttribArray = emscripten_glEnableVertexAttribArray = (index) => {
        glEnableVertexAttribArray_real(index);
        GLEmulation.enabledVertexAttribArrays[index] = 1;
        if (GLEmulation.currentVao) GLEmulation.currentVao.enabledVertexAttribArrays[index] = 1;
      };

      var glDisableVertexAttribArray_real = glDisableVertexAttribArray;
      glDisableVertexAttribArray = emscripten_glDisableVertexAttribArray = (index) => {
        glDisableVertexAttribArray_real(index);
        delete GLEmulation.enabledVertexAttribArrays[index];
        if (GLEmulation.currentVao) delete GLEmulation.currentVao.enabledVertexAttribArrays[index];
      };

      var glVertexAttribPointer_real = glVertexAttribPointer;
      glVertexAttribPointer = emscripten_glVertexAttribPointer = (index, size, type, normalized, stride, pointer) => {
        glVertexAttribPointer_real(index, size, type, normalized, stride, pointer);
