...
#if ASSERTIONS
    if (!ctx) err('emscripten_create_audio_context failed! Web Audio is not supported.');
#endif
    options >>= 2;
  ...
  emscripten_start_wasm_audio_worklet_thread_async: (contextHandle, stackLowestAddress, stackSize, callback, userData) => {
...
#if WEBAUDIO_DEBUG
      err(`emscripten_start_wasm_audio_worklet_thread_async() addModule() failed!`, e);
#endif
    ...
    if (!audioWorklet) {
      ...
      if (location.protocol == 'http:') {
        err(`AudioWorklets are not supported. This is possibly due to running the page over unsecure http:// protocol. Try running over https://, or debug via a localhost-based server, which should also allow AudioWorklets to function.`);
      } else {
        err(`AudioWorklets are not supported by current browser.`);
      }
#endif
...
#if WEBAUDIO_DEBUG
      dbg(`emscripten_start_wasm_audio_worklet_thread_async() addModule('audioworklet.js') completed`);
#endif
