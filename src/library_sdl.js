...
var LibrarySDL = {
  ...
  SDL_Init: (initFlags) => {
    ...
    SDL.keyboardState = malloc(0x10000); // Our SDL needs 512, but 64K is safe for older SDLs
  ...
  SDL_GetVideoInfo: () => {
    var ret = malloc({{{ C_STRUCTS.SDL_VideoInfo.__size__ }}});
  ...
  emscripten_SDL_SetEventHandler: (handler, userdata) => {
    ...
    if (!SDL.eventHandlerTemp) SDL.eventHandlerTemp = malloc({{{ C_STRUCTS.SDL_KeyboardEvent.__size__ }}});
  },
  ...
  Mix_PlayMusic: (id, loops) => {
    ...
    audio['onended'] = function() { if (SDL.music.audio == this) Mix_HaltMusic(); } // will send callback
  ...
  SDL_GL_SetSwapInterval: (state) => {
    emscripten_set_main_loop_timing({{{ cDefs.EM_TIMING_RAF }}}, state);
  },
