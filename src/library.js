...
`,
...
#if MAIN_MODULE == 1
  $makeDynCallLegacy__deps: ['$createDyncallWrapper'],
#endif
  $makeDynCallLegacy: (sig, ptr) => {
...
#endif
    return f.bind(null, ptr);
  },
#endif

  $getDynCaller__deps: ['$getWasmTableEntry',
#if DYNCALLS || !WASM_BIGINT
    '$makeDynCallLegacy'
#endif
  ],
  $getDynCaller: (sig, ptr) => {
#if DYNCALLS
    var func = makeDynCallLegacy(sig, ptr);
#else
    var func;
#if !WASM_BIGINT
    ...
    if (sig.includes('j')) {
      func = makeDynCallLegacy(sig, ptr);
    } else
#endif
    {
#if ASSERTIONS
...
#endif
      func = getWasmTableEntry(ptr);
    }
#endif // DYNCALLS

#if MEMORY64 || CAN_ADDRESS_2GB
    // With MEMORY64 and CAN_ADDRESS_2GB we sometimes need to wrap the resulting
    // function to handle pointer in the signature.
    function wrapForPointers(f) {
      return (...args) => {
#if MEMORY64
...
#endif
        var rtn = f(...args);
        return {{{ MEMORY64 ? 'Number(rtn)' : CAN_ADDRESS_2GB ? 'rtn >>> 0' : 'rtn' }}};
      };
    }
    if ({{{ MEMORY64 ? "sig.includes('p')" : "sig[0] == 'p'" }}}) {
      func = wrapForPointers(func);
    }
#endif // MEMORY64 || CAN_ADDRESS_2GB

#if ASSERTIONS
    // With ASSERTIONS enable we add a wrapper around the function to check
    // the number of arguments is correct.
    function wrapForAssertions(f) {
      return (...args) => {
        if (args.length) {
#if WASM_BIGINT
    ...
    func = wrapForAssertions(func);
#endif

    return func;
  },

  $dynCallDeps: ['$getDynCaller'],
  $dynCall: (sig, ptr, args = []) => getDynCaller(sig, ptr)(...args),
