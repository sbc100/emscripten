...
``-o <target>``
  ...
  Note: Linking to a file with no extension (or a file named ``a.out``) will
  cause the generated JavaScript file to be exectuable and included a ``#!`` line
  to make it runnable directly.
